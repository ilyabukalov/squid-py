...
class BrizoMock(object):
    ...
    def post(self, url, data=None, **kwargs):
        ...
        if url.endswith('initialize'):
            ...
            valid_signature = self.ocean_instance.verify_service_agreement_signature(did, sa_id, sa_def_id, consumer,
                                                                                     signature)
            assert valid_signature, 'Service agreement signature seems invalid.'
            if valid_signature:
                self.ocean_instance.execute_service_agreement(did, sa_def_id, sa_id, signature, consumer,
